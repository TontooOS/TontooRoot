...
echo "start-compositor: launching tontoo-compositor (Smithay)" >&2
...
if [ -x /usr/bin/tontoo-compositor ]; then
  exec /usr/bin/tontoo-compositor --udev
else
  echo "start-compositor: /usr/bin/tontoo-compositor not found" >&2
  exit 1
fi
