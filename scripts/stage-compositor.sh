#!/usr/bin/env bash
# Build the TontooOS compositor (Smithay, udev/DRM backend) and place the binary
...
if [[ ! -f "${compositor_dir}/Cargo.toml" ]]; then
  echo "stage-compositor: ${compositor_dir} is not a Smithay crate, skipping." >&2
  exit 0
fi
